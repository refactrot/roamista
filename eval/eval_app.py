from geographic_anal import Geographic_Anal
from semantic_similarity_anal import Semantic_Similarity_Anal
import os
...
def add_city(itineraries):
    for day in itineraries.values():
        city = day.pop(0)
        for i in range(len(day)):
            day[i] = day[i] + "+" + city.replace(" ", "+")
...
def main():
    ...
    while True:
        ...
        output1 = response_rag[response_rag.find("</think>") + len("</think>"): response_rag.find("```python")]
        ...
        links = google_maps_links(pois_dict_rag_clean)
        ...
        response_d = get_raw_response(user_query, " ".join(history[-5:]))
        output2 = response_d[response_d.find("</think>") + len("</think>"): response_d.find("```python")]
        pois_dict_d = parse_string(response_d[response_d.find("{") : response_d.find("}") + 1].replace("#", "").replace("*", "").replace("`", "").replace("\n", ""))
        pois_dict_d_clean = add_city(pois_dict_d)
        links_2 = google_maps_links(pois_dict_d_clean)
        print("Here are the google maps trajectories for each day.")
        for count in range(len(links_2)):
            print(f"Day {str(count + 1)} Google Maps Trajectory:", end = " ")
            print(links_2[count])
        print()
        s = Semantic_Similarity_Anal(user_query, output1, output2, pois_dict_rag, pois_dict_d)
        g = Geographic_Anal(pois_dict_rag, pois_dict_d)
        print(s.metrics)
        print(g.densities)
        print(g.backtracking)
